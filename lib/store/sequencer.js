...
const ARP_TOGGLE      = 'sequencer/arp/TOGGLE'
const ARP_EXTEND      = 'sequencer/arp/EXTEND'
const ARP_SET_NAME    = 'sequencer/arp/SET_NAME'
const ARP_SET_OFFSET  = 'sequencer/arp/SET_OFFSET'
const ARP_SET_TRIGGER = 'sequencer/arp/SET_TRIGGER'
...
export const toggleArpPanelAction = () => ({
	type: ARP_TOGGLE
})

export const arpExtendAction = (index) => ({
	type: ARP_EXTEND, index
})

export const arpSetNameAction = (index, name) => ({
	type: ARP_EXTEND, index, name
})

export const arpSetTriggerAction = (index, trigger) => ({
	type: ARP_SET_TRIGGER, index, trigger
})

export const arpSetOffsetAction = (index, pos, offset) => ({
	type: ARP_SET_OFFSET, index, pos, offset
})
...
export default function reducer (state = initialState, action = {}) {
	...
	case ARP_TOGGLE:
		newState.arpvisible = !state.arpvisible
		return newState

	case ARP_EXTEND:
		newState.arps[action.index].offsets = state.arps[action.index].offsets.slice()
		newState.arps[action.index].offsets.push(0)
		newState.arps[action.index].active = [...Array(state.arps[action.index].offsets.length + 1)]
		return newState
	...
	case ARP_SET_OFFSET:
		newState.arps[action.index].offsets[action.pos] = action.offset
		return newState

	case ARP_SET_TRIGGER:
		newState.arps[action.index].trigger = action.trigger
		return newState
