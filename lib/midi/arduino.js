...
import {setSyncModeAction} from 'lib/store/midi'
...
function Sysex (event) {
	...
	case 0x40:
		...
		store.dispatch(setSyncModeAction(clock.SyncMode.Arduino))
		break
	default:
...
function Stop () {
	outputs[detectedPort].send([
		0xf0, 0x7d,            // sysex, non commercial / educationall manufacturer id.
		0x2a, 0x4d,            // my own little secret magic number id for this router.
		0x04,                  // COMMAND 0x04 = stop
		0xf7
	])
}

var arduino = {Init, Sysex, Scan, SetBPM, UpdateRoute, Play, Stop}
