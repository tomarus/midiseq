...
function MidiIN (msg) {
	switch (msg.status) {
	case 0xf8: // tick
		if (syncmode === SyncMode.Forward || syncmode === SyncMode.Arduino) {
			clockout([0xf8])
		}
		if ((syncmode === SyncMode.Receive || syncmode === SyncMode.Forward || syncmode === SyncMode.Arduino) && playing) {
			handleTick()
		}
		calcBPM()
		break
